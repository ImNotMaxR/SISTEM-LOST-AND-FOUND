...
public class ReportDetailFrame extends JDialog {
    ...
    private static final Color PRIMARY_ACTION = UserDashboardComponents.PRIMARY;
    ...
    private JPanel createContent(Report report) {
        ...
        root.add(UserDashboardComponents.section("Detail Laporan", getHeaderSubtitle(report)), BorderLayout.NORTH);
    ...
    private JPanel createInfoCard(Report report) {
        ...
        GridBagConstraints gbc = UserDashboardComponents.contentConstraints();
        gbc.gridy = 0;
        panel.add(UserDashboardComponents.label("Informasi Barang", 18, Font.BOLD, UserDashboardComponents.TEXT_DARK), gbc);
        gbc.gridy = 1;
    ...
    private JPanel createInfoCell(String label, String value) {
        ...
        cell.add(createText(value, 13, Font.BOLD, UserDashboardComponents.TEXT_DARK), gbc);
        return cell;
    }
    ...
    private JPanel createDescriptionBox(String title, String value) {
        ...
        GridBagConstraints gbc = UserDashboardComponents.contentConstraints();
        gbc.gridy = 0;
        box.add(UserDashboardComponents.label(title, 12, Font.BOLD, UserDashboardComponents.TEXT_MUTED), gbc);
        gbc.gridy = 1;
        gbc.insets = new Insets(8, 0, 0, 0);
        box.add(createText(value, 13, Font.BOLD, UserDashboardComponents.TEXT_DARK), gbc);
        return box;
    }
    ...
    private JPanel createActionCard(Report report) {
        ...
        panel.add(createText("Semua user dapat melihat status dan klaim untuk transparansi pengelolaan barang kampus.", 12, Font.PLAIN, UserDashboardComponents.TEXT_MUTED), gbc);
        gbc.gridy = 2;
        gbc.weighty = 1;
        JPanel spacer = new JPanel();
        spacer.setOpaque(false);
    ...
    private JPanel createNoticeBox(String text) {
        ...
        box.setBorder(BorderFactory.createCompoundBorder(
                new UserDashboardComponents.RoundedLineBorder(new Color(191, 219, 254), 14, 1),
                BorderFactory.createEmptyBorder(10, 12, 10, 12)
        ));
        box.add(createText(text, 12, Font.BOLD, UserDashboardComponents.PRIMARY_DARK), BorderLayout.CENTER);
        return box;
    }
    ...
    private JPanel createStatusPill(String status) {
        UserDashboardComponents.RoundedPanel pill = new UserDashboardComponents.RoundedPanel(new Color(255, 237, 213), 14);
        pill.setLayout(new BorderLayout());
        pill.setBorder(BorderFactory.createEmptyBorder(10, 14, 10, 14));
        pill.add(UserDashboardComponents.label("Status: " + status, 12, Font.BOLD, new Color(194, 65, 12)), BorderLayout.CENTER);
        return pill;
    }
    ...
    private JButton createActionButton(String text, boolean primary) {
        ...
        button.setForeground(primary ? Color.WHITE : UserDashboardComponents.TEXT_DARK);
        button.setUI(new javax.swing.plaf.basic.BasicButtonUI() {
            ...
            public void paint(Graphics graphics, JComponent component) {
                ...
                if (!button.isEnabled()) {
                    ...
                    fill = button.getModel().isRollover() ? UserDashboardComponents.PRIMARY_DARK : PRIMARY_ACTION;
    ...
    private String getReportLocation(Report report) { return report != null ? UserDashboardComponents.location(report) : "-"; }
    private String getReportItemStatus(Report report) { return report != null && report.getItem() != null ? report.getItem().getStatus().name() : "-"; }
    private String getReportDate(Report report) { return report != null ? UserDashboardComponents.date(report) : "-"; }
    ...
    private static class DetailPhotoPanel extends JPanel {
        ...
        private void paintFallback(Graphics2D g2, int width, int height) {
            g2.setPaint(new java.awt.GradientPaint(0, 0, UserDashboardComponents.PRIMARY_LIGHT, width, height, UserDashboardComponents.PRIMARY_DARK));
            g2.fillRoundRect(0, 0, width, height, 18, 18);
