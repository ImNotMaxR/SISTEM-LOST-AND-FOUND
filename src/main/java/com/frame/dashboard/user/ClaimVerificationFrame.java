...
public class ClaimVerificationFrame extends JDialog {
    ...
    private JPanel createMainPanel() {
        ...
        root.add(UserDashboardComponents.section(TITLE, SUBTITLE), BorderLayout.NORTH);
    ...
    private JPanel createItemSummary() {
        UserDashboardComponents.RoundedPanel panel = new UserDashboardComponents.RoundedPanel(new Color(248, 250, 252), 16);
        panel.setLayout(new GridBagLayout());
        panel.setBorder(BorderFactory.createCompoundBorder(
                new UserDashboardComponents.RoundedLineBorder(new Color(230, 236, 244), 16, 1),
        ...
        panel.add(UserDashboardComponents.label("Barang Yang Diklaim", 12, Font.BOLD, UserDashboardComponents.TEXT_MUTED), gbc);
        gbc.gridy = 1;
        gbc.insets = new Insets(8, 0, 0, 0);
        panel.add(UserDashboardComponents.label(item == null ? "-" : item.getName(), 16, Font.BOLD, UserDashboardComponents.TEXT_DARK), gbc);
        gbc.gridy = 2;
        gbc.insets = new Insets(4, 0, 0, 0);
        String category = item == null || item.getCategory() == null ? "Tanpa kategori" : item.getCategory().getName();
        panel.add(UserDashboardComponents.label(category, 12, Font.PLAIN, UserDashboardComponents.TEXT_MUTED), gbc);
        return panel;
    }
