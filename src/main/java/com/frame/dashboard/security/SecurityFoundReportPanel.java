...
public class SecurityFoundReportPanel extends JDialog {
    ...
    private java.util.ArrayList<com.model.LostReport> pendingLostReports;
    
    private PhotoPreviewPanel photoPreviewPanel;
    private File selectedPhotoFile;

    public SecurityFoundReportPanel(User user, ReportManager reportManager, Runnable onReportSaved) {
    ...
    public SecurityFoundReportPanel() {
    ...
    private JPanel createMainPanel() {
        ...
        btnSubmit.addActionListener(e -> submitFoundReport());
    ...
    private JPanel createFormPanel() {
        ...
        String[] lrOptions = new String[pendingLostReports.size() + 1];
        lrOptions[0] = "-- Item Baru (Tidak ada kecocokan) --";
        for (int i = 0; i < pendingLostReports.size(); i++) {
            com.model.LostReport lr = pendingLostReports.get(i);
            lrOptions[i + 1] = lr.getReportId() + " - " + lr.getItem().getName() + " (" + lr.getUser().getName() + ")";
        }
        
        lostReportComboBox = new JComboBox<>(lrOptions);
        lostReportComboBox.setPreferredSize(new Dimension(300, 45));
        lostReportComboBox.setFont(new Font("Poppins", Font.BOLD, 13));
        lostReportComboBox.setForeground(UserDashboardComponents.TEXT_DARK);
        lostReportComboBox.setBackground(Color.WHITE);
        lostReportComboBox.setFocusable(false);
        lostReportComboBox.setBorder(BorderFactory.createCompoundBorder(
                new UserDashboardComponents.RoundedLineBorder(new Color(230, 230, 230), 16, 1),
                BorderFactory.createEmptyBorder(2, 10, 2, 0)
        ));
        
        lostReportComboBox.setUI(new BasicComboBoxUI() {
            @Override
        ...
        lostReportComboBox.addActionListener(e -> {
            ...
            if (idx > 0) {
                com.model.LostReport lr = pendingLostReports.get(idx - 1);
                itemNameField.setText(lr.getItem().getName());
                itemDescriptionArea.setText(lr.getItem().getDescription());
                for (int i = 0; i < categoryComboBox.getItemCount(); i++) {
                    if (categoryComboBox.getItemAt(i).getCategoryID().equals(lr.getItem().getCategory().getCategoryID())) {
        ...
        storageLocationField = createTextField("Contoh: Lemari A1 Security", 100);
        addField(panel, gbc, 4, "Lokasi Penyimpanan Gudang*", storageLocationField, 0.0);

        reportDescriptionArea = createTextArea("Kronologi singkat penemuan", 500);
        addField(panel, gbc, 5, "Deskripsi laporan*", createTextAreaScroll(reportDescriptionArea), 1.0);

        categoryComboBox = createCategoryComboBox();
        addField(panel, gbc, 6, "Pilih kategori*", categoryComboBox, 0.0);
